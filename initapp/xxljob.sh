...
function xxljob211 {
  wget https://codeload.github.com/xuxueli/xxl-job/zip/refs/tags/2.1.1 -O /data/server/xxl-job-2.1.1.zip;
  # 解压
  unzip /data/server/xxl-job-2.1.1.zip;

  # 修改配置：xxl-job/xxl-job-admin/src/main/resources/application.properties
  # 127.0.0.1:3306/xxl_job替换成自己的
  replace_db_url="10.100.64.154:3306"
  replace_db_database="xxl_job?useUnicode"
  replace_db_username="spring.datasource.username=root"
  replace_db_password="spring.datasource.password=root"
  # 替换数据库配置
  sed -ig "s/127.0.0.1:3306/"${replace_db_url}"/" /data/server/xxl-job-2.1.1/xxl-job-admin/src/main/resources/application.properties;
  sed -ig "s/xxl_job?useUnicode/"${replace_db_database}"/" /data/server/xxl-job-2.1.1/xxl-job-admin/src/main/resources/application.properties;
  sed -ig "s/spring.datasource.username=root/"${replace_db_username}"/" /data/server/xxl-job-2.1.1/xxl-job-admin/src/main/resources/application.properties;
  sed -ig "s/spring.datasource.password=root_pwd/"${replace_db_password}"/" /data/server/xxl-job-2.1.1/xxl-job-admin/src/main/resources/application.properties;
  cat /data/server/xxl-job-2.1.1/xxl-job-admin/src/main/resources/application.properties;
  # 进入安装目录构建
  cd /data/server/xxl-job-2.1.1;
  mvn clean package;
  # 启动
  nohup java -jar /data/server/xxl-job-2.1.1/xxl-job-admin/target/xxl-job-admin-2.1.1.jar > /data/server/xxl-job-start.log &

  # http://192.168.163.149:8080/xxl-job-admin
}